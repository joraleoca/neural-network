...
class Tokenizer:
    ...
    _PAD: ClassVar[str] = "<pad>"
    _SOS: ClassVar[str] = "<sos>"
    _EOS: ClassVar[str] = "<eos>"
    _UNK: ClassVar[str] = "<unk>"

    def __init__(self, vocab: list[str]) -> None:
        self.vocab = [self._PAD, self._SOS, self._EOS, self._UNK] + vocab
    ...
    def __call__(
        ...
        if add_sos:
            tokens.append(self.word2idx[self._SOS])

        unk_token = self.word2idx[self._UNK]
        tokens.extend(self.word2idx.get(w, unk_token) for w in words)

        if add_eos:
            tokens.append(self.word2idx[self._EOS])

        if len(tokens) < max_size:
            tokens += [self.word2idx[self._PAD]] * (max_size - len(tokens))
